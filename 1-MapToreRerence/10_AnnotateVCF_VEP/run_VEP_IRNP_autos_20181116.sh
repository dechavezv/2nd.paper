...
-i IRNP_44_joint_chr$(printf %02d $SGE_TASK_ID)_TrimAlt_Annot.vcf.gz -o STDOUT \
--stats_file IRNP_44_joint_chr$(printf %02d $SGE_TASK_ID)_TrimAlt_Annot_VEP_stats.html \
--sift b --species canis_familiaris --canonical --allow_non_variant --symbol --force_overwrite | \
sed 's/ /_/g' | $BGZIP > IRNP_44_joint_chr$(printf %02d $SGE_TASK_ID)_TrimAlt_Annot_VEP.vcf.gz

$TABIX -p vcf IRNP_44_joint_chr$(printf %02d $SGE_TASK_ID)_TrimAlt_Annot_VEP.vcf.gz
