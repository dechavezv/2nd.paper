...
-i IRNP_35_joint_chr$(printf %02d $SGE_TASK_ID)_TrimAlt_filtered.vcf.gz -o STDOUT \
--stats_file IRNP_35_joint_chr$(printf %02d $SGE_TASK_ID)_TrimAlt_filtered_VEP_stats.html \
--sift b --species canis_familiaris --canonical --allow_non_variant --symbol --force_overwrite | \
$BGZIP > IRNP_35_joint_chr$(printf %02d $SGE_TASK_ID)_TrimAlt_filtered_VEP.vcf.gz

$TABIX -p vcf IRNP_35_joint_chr$(printf %02d $SGE_TASK_ID)_TrimAlt_filtered_VEP.vcf.gz
