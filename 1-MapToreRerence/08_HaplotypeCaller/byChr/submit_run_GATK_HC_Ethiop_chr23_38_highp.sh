...
for i in {23..38}; 
	...
	echo "#$ -l h_rt=24:00:00,h_data=16G,h_vmem=26G,arch=intel*"
	echo "#$ -t 01-1:1"
	echo "#$ -N HC_EW_${i}"
	...
	echo
	echo "export BAM=\$(ls Csmi\$(printf "%02d" "\$SGE_TASK_ID")_Aligned.MarkDup_Filtered.bam)"
	...
	echo "-L chr${i} \\"
	echo "-I \${BAM} \\"
	echo "-o /u/scratch/d/dechavez/Ethiopian/GVCFs/\${ID}_chr${i}.g.vcf.gz"
	
	) > "run_GATK_HC_EthW_chr23_38_highp.sh"

	qsub run_GATK_HC_EthW_chr23_38_highp.sh
