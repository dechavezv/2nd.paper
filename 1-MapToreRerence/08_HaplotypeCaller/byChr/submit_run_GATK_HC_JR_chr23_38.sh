...
for i in {23..38}; 
	...
	do (
	echo "#! /bin/bash"
	echo "#$ -wd /u/scratch/d/dechavez/NA.Genomes"
	echo "#$ -l h_rt=24:00:00,h_data=8G,h_vmem=24G,arch=intel*"
	echo "#$ -t 26-43:1"
	echo "#$ -N HC_NA_Clup_${i}"
	echo "#$ -o /u/scratch/d/dechavez/NA.Genomes/log/reports"
	echo "#$ -e /u/scratch/d/dechavez/NA.Genomes/log/reports"
	echo "#$ -m abe"
	echo "#$ -M dechavezv"
	echo
	...
	echo
	echo "cd /u/scratch/d/dechavez/NA.Genomes"
	echo
	echo "export BAM=\$(ls \$(printf "%02d" "\$SGE_TASK_ID")_IRNP_*Aligned_MarkDuplicates_Filtered.bam)"
	echo "export ID=\${BAM%_Aligned_MarkDuplicates_Filtered.bam}"
	echo
	...
	echo "-L chr${i} \\"
	echo "-I \${BAM} \\"
	echo "-o /u/scratch/d/dechavez/NA.Genomes/\${ID}_chr${i}.g.vcf.gz"
	
	) > "run_GATK_HC_JR_chr23_38_highp.sh"

	qsub run_GATK_HC_JR_chr23_38_highp.sh
