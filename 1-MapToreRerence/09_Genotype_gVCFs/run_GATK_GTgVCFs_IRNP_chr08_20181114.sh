...
coords=$(cat $(ls chr08_coords.list_* | head -n ${SGE_TASK_ID} | tail -n 1))

cd /u/flashscratch/j/jarobins/gVCFs/chr08
...
-stand_call_conf 0 \
-L ${coords} \
$(for j in *_IRNP_*_chr08.g.vcf.gz; do echo "-V ${j} "; done) \
-o IRNP_44_joint_chr08_$(printf "%02d" "$SGE_TASK_ID").vcf.gz 
