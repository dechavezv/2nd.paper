...
for line in parsevcf.fetch(chromo,start_pos,end_pos):
	...
	#if ('.' in line[-8]): continue 
	sites_passing+=1
	
	#All sites homozygous reference
        #If two alleles add 1
        if ('.' in line[-1] or '0/0' in line[-1]) and ('.' in line[-2] or '0/0' in line[-2]) and ('.' in line[-3] or '0/0' in line[-3]) and ('.' in line[-4] or '0/0' in line[-4]) and  ('.' in line[-5] or '0/0' in line[-5]) and  ('.' in line[-6] or '0/0' in line[-6]) and ('.' in line[-7] or '0/0' in line[-7]) and  ('.' in line[-8] or '0/0' in line[-8]) and ('.' in line[-9] or '0/0' in line[-9]) and ('.' in line[-10] or '0/0' in line[-10]):		
		SegregationSites.append(float(0))		
	else:
		SegregationSites.append(float(1))

if numpy.sum(SegregationSites) == 0:
	Diver=0
	print('%s\t%d\t%d\t%f\t%d\t%s' % (chromo,start_pos,end_pos,Diver,sites_passing,EnsemblID))

    
elif numpy.sum(SegregationSites) > 0:
	Diver=0
	#print(sites_passing)
	#print(numpy.sum(SegregationSites))
#	print(float((numpy.sum(SegregationSites)/sites_passing)))
	Diver=float((numpy.sum(SegregationSites)/10)/sites_passing)
	print('%s\t%d\t%d\t%f\t%d\t%s' % (chromo,start_pos,end_pos,Diver,sites_passing,EnsemblID))
