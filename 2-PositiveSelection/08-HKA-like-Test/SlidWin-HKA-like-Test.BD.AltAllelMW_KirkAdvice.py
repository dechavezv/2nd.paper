...
def computedPolymor(AF_all,DS_all,sites_passing,sites_present,sites_polymorphic):
	...
	#print('%s\t%d\t%d\t%s\t%f\t%d' % (chromo,window_start,window_end,str('Diversity'),Polymor,sites_passing))
        ...
        Diver=(numpy.sum(DS_all)/sites_passing)
...
def fetch_and_calc(chromo,start_pos,end_pos):
	...
	DS_all=[]
        ...
        for line in parsevcf.fetch(chromo,start_pos,end_pos):
                line = line.split('\t')
                sites_present+=1
                if ('PASS' not in line[6]) or ('.' in line[-1]) or ('.' in line[-2]) or ('.' in line[-3]) or ('.' in line[-4]) or ('.' in line[-5]) or ('.' in line[-6]) or ('.' in line[-7]) or ('.' in line[-8]) or ('.' in line[-9]): continue
                sites_passing+=1
                #print(line[-4:])
                if line[4]=='.': continue
                sites_polymorphic+=1
