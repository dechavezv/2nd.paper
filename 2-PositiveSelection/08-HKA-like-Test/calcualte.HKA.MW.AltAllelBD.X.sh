...
python ${SCRIPT_DIR}/SlidWin-HKA-like-Test.MW.AltAllelBD.py ../bsve_bcbr_chr${i}_TrimAlt_Annot_Mask_Filter.vcf.gz \
100000 10000 chr${i} > bcbr_AltAllelBD_chr${i}_TrimAlt_Annot_Mask_Filter.HKA.txt
echo '********** Done calculating HKA-like test ***********'

echo '********** Extracting genes within windows ***********'
python ${SCRIPT_DIR}/Genes_within_HKA.py bcbr_AltAllelBD_chr${i}_TrimAlt_Annot_Mask_Filter.HKA.txt ${OrtoBed}
